class CharacterChargeBar extends DrawableObject {
  ...
  maxSegments = 5;
  segments = 0; // starts empty
  // Default size; will be aligned to health bar dimensions by world
  width = 210;
  height = 20;
  x = 40;
  y = 84; // below block bar by default
  ...
  getSlotPath(i) {
    const max = this.maxSegments || 5;
    const isFull = this.segments > i; // fill left->right
  ...
  drawAt(ctx, dx, dy) {
    ...
    // 1) Draw all empties as the frame
    ...
    // 2) Overlay full segments up to current charge count
